...
return(<div className="flex my-1 ml-1">
    ...
    {allSizesNames.map((v,i,arr)=>{
      if(selectedSize===v){
        ...
        return <button id="non_recomended" key={i} className="rounded-lg bg-black text-white shadow-lg py-2 mx-2 px-2">{v}</button>
      }
         ...
         else return <button key={i} id="non_recomended" className="rounded-lg bg-white shadow-lg py-2 mx-2 text-black px-2" onClick={()=>changeSize(i)}>{v}</button>
      })}
